# ElevenLabs Voice Agent — System Prompt & Voice

> Authoritative snapshot fetched live from the deployed ElevenLabs Conversational AI
> agent on 2026-05-31. This is the prompt that actually runs in production.
> The draft `prompts/system-prompt-v1.md` is kept in sync with this deployed prompt.

## Agent configuration

| Field | Value |
|---|---|
| Agent name | `Little Red Beanie` |
| Agent ID | `agent_6901ksvqwxtvfrxtbz43hpd0hbsj` |
| LLM backend | `claude-opus-4-7` |
| max_tokens | `-1` (unbounded) |
| Language | `en` |
| First message | `Helloo there. I am Little Red Beanie. What is your name?` |

The doubled "o" in "Helloo" is intentional — it stops the streaming TTS from clipping the
leading consonant of the very first word.

## Voice

The deployed voice is the ElevenLabs library voice
**"Goofy - Nasal, Fast-Paced and Energetic"** (`BRruTxiLM2nszrcCIpz1`).

> **Note:** this is *not* the warm, calm custom voice described in the original design
> brief (`output/voice-design-prompt.md` / `CLAUDE.md`). The deployed agent currently uses
> this stock character voice, set during the 2026-05-30 demo prep. The original
> Voice-Design intent (warm, gentle, young-adult-male puppet voice) is preserved in
> `output/voice-design-prompt.md` for a future re-recording.

| Field | Value |
|---|---|
| Voice name | **Goofy - Nasal, Fast-Paced and Energetic** |
| Voice ID | `BRruTxiLM2nszrcCIpz1` |
| Category | `professional` (ElevenLabs library voice) |
| Accent | Canadian |
| Age | Young |
| Gender | Male |
| Descriptive | Upbeat |
...
### TTS settings (fetched live from the agent, 2026-05-31)
...
| Speed | `1.0` |
| Optimize streaming latency | `3` (reduced from 4 — level 4 cut off the first word "Hi") |

---

## Deployed system prompt (verbatim)
